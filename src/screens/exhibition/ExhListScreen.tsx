...
import {fetchSearchExh} from '~/api/exhibition';
...
const ExhListScreen = () => {
  ...
  const { data, isLoading, isError, isSuccess, refetch } =  useQuery('AllExhList', fetchAllExh, {
    onSuccess: (data) => console.log('[MyExhListScreen] success fetch AllExhLists',data),
  ...
  return (
    <Container>
      ...
      <Header title={'전시회'}>
          <IconsView>
           <TouchableOpacity>
            ...
            <TouchableOpacity  onPress={() => navigation.navigate('MyAddExhibition')}>
              <AnotherSearchIcon/>
            </TouchableOpacity>
            <TouchableOpacity>
              <CalendarIcon/>
      ...
      <ScrollView style={{ flex: 1 }} scrollEventThrottle={200}>
        {data.map((item: any, index:number) => ( <Contents key={item.exhId}><TouchableOpacity><ExhItemView  {...item}>
          </ExhItemView></TouchableOpacity>
              <EmptyHeartContent>
                <HeartContent> 
                <TouchableOpacity onPress={()=>onPressHeart(item.exhId)} > 
                  {hearts.length !== 0 && hearts[index].favoriteExh? <FullHeart /> : <EmptyHeart />}
                </TouchableOpacity>
                </HeartContent>  
            </EmptyHeartContent>
        </Contents>))}
